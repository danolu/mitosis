...
<body data-base-url="{{url('admin/')}}">
  ...
  <div class="main-wrapper" id="app">
    <livewire:sidebars.admin /> 
    <div class="page-wrapper">
      <livewire:headers.admin />   
      <div class="page-content">
        {{ $slot }}
      </div>
      @persist('admin_footer')
      <livewire:footers.admin /> 
      @endpersist  
    </div>
  </div>
  ...
  <script>
    document.addEventListener('closeModal', function(e){
      const modal = bootstrap.Modal.getInstance("#" + e.detail.name.toString());
      modal.hide();
    });
  </script>
  <script>
...
<script>
  document.addEventListener('feather', function(e){
    setTimeout(function() {
      feather.replace();
      }, 400)
